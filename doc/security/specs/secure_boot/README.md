...
2. Load the "manifest" for `ROM_EXT`, which includes the start and end address of `ROM_EXT` code, a cryptographic signature, a public key modulus, and "selector bits" for hardware information.
    ...
    * There are two slots for `ROM_EXT` implementations. First the ROM will choose a primary boot slot and a secondary one for `ROM_EXT`. If the signature verification for the primary slot fails, then ROM will attempt to boot from the secondary slot.
    * ROM determines the primary boot slot by choosing the `ROM_EXT` implementation with the higher security version in the manifest. In case the security version is equal for both implementations, ROM will pick the one with the higher major/minor version tuple.
